...
class mysqlOperations:
        ...
        try:
            while True:
                ...
                for i in all_tables:
                    ...
                    logging.debug("final query for create : {} ".format(query))
                ...
                if choice.upper() == "Y":  #checking user decision whether to proceed with table creation or not
                    ...
                    logging.debug("Choice by the user : {}".format(choice))
                    break #break while
    ...
    def insert_table(self):
        ...
        try:
            ...
            logging.debug("Column names of {} table : {}".format(table_name,columns_name))
            ...
            logging.debug("Final insert query:".format(final_query))
            ...
            if user_choice == 1: # insert with csv starts here
                ...
                with open(input_file,'r') as fin:
                    ...
                    # all_value=[]
                    for row in csvfile:
                        logging.debug("Rows inserted {}:".format(row))
                ...
                self.send_mail("Insert",table_name,self.dbname) #calling sending mail function
                ...
                for i in range(len(columns_name)):
                    ...
                    logging.debug("User values for insert to {} column {}: ".format(columns_name[i],value))
                ...
                logging.debug("input received for input method inside insert operation : {}".format(user_choice))
    ...
    def print_data(self,table):
        ...
        try:
            ...
            logging.debug("final select query: {}".format(query))
    ...
    def delete_data(self):
        ...
        try:
            ...
            if criteria == 1:
                query=f"""DELETE FROM {input_table}""" #query to delete all data
                values=()
            elif criteria == 2:
                ...
                query = f"""DELETE FROM {input_table} WHERE {column_name}=%s""" # query for delete based on condition
                values=(column_value,)
            else:
                print("Invalid input, Delete operation terminated !!")
                logging.debug("Entered invalid input by the user for delete criteria , {} ".format(criteria)) #logging
            if query != "":
                self.mycursor.execute(query,values) #executing delete query
    ...
    def update_table(self):
        ...
        try:
            ...
            query = f"""UPDATE {input_table} SET {update_column}=%s WHERE {update_condition_column}=%s"""
            logging.debug("Final update query :{}".format(query))
            values=(update_column_value,update_condition_value)
            self.mycursor.execute(query,values) #executing query
        ...
        if my_sql.my_connection != None: #checking if connection is open
            while True :
                ...
                if choice.upper() == "Y":
                    ...
                    logging.debug("User entered choice for continue: {}".format(choice))
                    break
    except Exception as e:
        logging.critical(e)
    finally:
